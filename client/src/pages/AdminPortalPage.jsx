...
const NurseApprovalCard = ({ nurse, onAction }) => {
  const handleApprove = async () => {
    if (
      !window.confirm(
        `Are you sure you want to approve Nurse ${nurse.first_name} ${nurse.last_name}?`
      )
    )
      return;
    try {
      await axios.post("/api/admin/approve-nurse", {
  ...
  const handleDecline = async () => {
    if (
      ...
      return;
    try {
      await axios.post("/api/admin/decline-nurse", {
        employee_id: nurse.employee_id,
      });
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md border flex justify-between items-center">
      <div>
        ...
        <p className="text-sm text-gray-500">
          {nurse.email} | License: {nurse.license_number}
        </p>
      </div>
      <div className="flex space-x-2">
        <button onClick={handleDecline} className="btn btn-error btn-sm">
...
const AdminPortalPage = () => {
  ...
  const [activeTab, setActiveTab] = useState("doctors"); // 'doctors' or 'nurses'
  ...
  const handleAction = (employeeId, type) => {
    if (type === "doctors") {
      setPendingDoctors((prev) =>
        prev.filter((doc) => doc.employee_id !== employeeId)
      );
    } else if (type === "nurses") {
      setPendingNurses((prev) =>
        prev.filter((nurse) => nurse.employee_id !== employeeId)
      );
    }
  };
