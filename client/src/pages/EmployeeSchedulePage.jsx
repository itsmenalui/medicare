...
const AppointmentCard = ({ appointment, onSelectAppointment }) => {
  ...
  // FINAL FIX: The backend now sends a full ISO string (e.g., "2025-07-30T05:00:00.000Z").
  // The Date constructor can now parse this directly and correctly without manipulation.
...
const EmployeeSchedulePage = () => {
  ...
  // useEffect to call fetchAvailability
  useEffect(() => {
  ...
  const handleRemoveSlot = async (timeSlot) => {
    ...
    try {
      ...
      // Refresh availability from the server to reflect the change
      fetchAvailability();
            ...
            <div className="space-y-6">
              {Object.keys(groupedSchedule).length > 0 ? (
                Object.keys(groupedSchedule).map((date) => (
                  <div key={date} className="bg-white p-6 rounded-2xl shadow-md">
